...
for k, report in enumerate(reports):
    ...
    if status_a_final == 'Failed A':
        ...
        if status_b == 'Passed B': # report is SAFE!!!
            nb_reports_safe += 1
            # print(f"report {report} is SAFE (passed b)!!! => nb_reports_safe = {nb_reports_safe}")
            continue # go to next report
            ...
            if dampener == True:
                ...
                if  b_[1] == len(report_list) - 2: # we use dampener once to remove last element failing B
                    status_b_final = 'Passed B'
                    nb_reports_safe += 1
                    dampener = True
                    # print(f"report {report} is SAFE (passed b with dampener LEN)!!! => nb_reports_safe = {nb_reports_safe}")
                elif b_[1] == 0: # we remove first element (because same direction thanks to checkA) and checkB for the rest of elements 
                    print(report_list)
                    print(report_list[1:])
                    print(checkB(report_list[1:]))
                    if checkB(report_list[1:])[0] == 'Passed B' :
                        status_b_final = 'Passed B'
                        nb_reports_safe += 1
                        dampener = True
                        print(f"report {report} is SAFE (passed b with dampener 0)!!! => nb_reports_safe = {nb_reports_safe}")
...
'''
DEBUG: problem with passed b with dampener
Examples: 
report 66 71 72 74 78 81 is SAFE (passed b with dampener)!!! => nb_reports_safe = 160
report 62 58 55 53 46 41 is SAFE (passed b with dampener)!!! => nb_reports_safe = 162
...
AVANT:
                ...
                if b_[1] == 0 or b_[1] == len(report_list) - 2:
                    status_b_final = 'Passed B'
                    nb_reports_safe += 1
                    print(f"report {report} is SAFE (passed b with dampener)!!! => nb_reports_safe = {nb_reports_safe}")
                    dampener = True
                else: # même si dampener pas utilisé, dans une liste strictement (dé)croissante, la distance sera trop élevée
                    continue # go to next report
                    
APRES: (282 -> 294, too low) => we filtered too many cases, we can see in results no cases with 'passed b with dampener 0')
